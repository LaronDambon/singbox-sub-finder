...
def parse(data):
    ...
    try: #fuck
        param = param.split('?')[0]
        matcher = tool.b64Decode(param) # сохраняем '/' и проверяем, можно ли декодировать
    except:
        param = param.split('/')[0].split('?')[0] # если декодирование не удалось, значит '/' не является содержимым base64
    if param.find('@') > -1:
